...
public class Robot extends SampleRobot {
	...
	final int Pnematic_Open = 3;	//stick 2
	final int SlowHold = 8;			//stick 1
	final int Lift_Intake = 3; 		//stick 1 
	final int auto360 =10;			//stick 1
    ...
    public Robot() {
	    ...
	    JoystickButton do360 = new JoystickButton(stick2, auto360);

	    
	    do360.whenPressed(new InstantCommand(){
	    	@Override
	    	protected void execute() {
	    		m_drive.arcadeDrive(0.5, 1);
...
//    }
		...
		if(gameData.charAt(0) == 'L'){
    		...
    		while(running2){
        		forkLiftMech.set(-0.6);
    			if((System.currentTimeMillis() - start2) >= 3500){
    				running2 = false;
    				running3 = true;
            		forkLiftMech.set(0);
    			}
    		...
    		while(running3){
    			this.drive(0.3, 0, false);
    			if((System.currentTimeMillis() - start3) >= 2000){
    				running3 = false;
    				pause = true;
    			}
    		}
    		...
    		while(pause){
    			this.drive(0.05, 0, false);
    			if((System.currentTimeMillis() - startPause) >= 500){
    				pause = false;
    				running4 = true;
    				fly1.set(0.5);
    				fly2.set(0.5);
    		...
    		while(running4){
    			...
    			if((System.currentTimeMillis() - start4) >= 1500){
    				running4 = false;
    				fly1.set(0);
    				fly2.set(0);
    			}
...
//        	arcadeDrive.setSafetyEnabled(false);
    	...
    	while (isOperatorControl() && isEnabled()) {
        	...
        	boolean flywheelsOnPlace = stick.getRawButton(2);
